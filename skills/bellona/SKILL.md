...
description: >-
  ...
  Use when adding oxlint.config.ts jsPlugins, turning on bn-* rules, debugging bellona
  lint, writing a new bellona rule with defineBellonaRule and createOnce, pinning oxlint
  for JS plugins, or when the user mentions bellona, Oxlint JS plugins, bn-max-classes,
  bn-prefer-effect-fn, bn-no-native-html, or opt-in Oxlint plugins.
---
...
Use this skill when the work is: install/config, enable/tune `bn-*` rules, interpret a bellona diagnostic, write code that satisfies a rule, or add a plugin/rule to the bellona package itself.
...
- Subpath import = plugin load. Rule id = `<plugin-name>/bn-<slug>` (example: `js/bn-max-classes`). Plugin `meta.name` is the last export segment: `js`, `react`, `base-ui`, `zod`, `tanstack-router`, `elysia`, `effect`.
...
- Zod / TanStack Router / Elysia / Effect rules skip files that do not import the matching package (Elysia `bn-no-route-factory` is the exception: path-gated, no import required).
...
- After config edits, confirm the rule id is `<name>/bn-<slug>` and the plugin is in `jsPlugins`.
