...
name: bellona
description: >-
  Install, configure, and enable bellona 0.4.2 Oxlint JS plugins (bellona/js, bellona/react,
...
Bellona is a set of **opt-in Oxlint JS plugins** (this skill matches **bellona 0.4.2**: 8 plugins, 88 rules). Install the package, load only the subpaths you need, then turn rules on by id. **Nothing is enabled by default.** There is no recommended config.
