...
Plugin name: `elysia`. Ids: `elysia/bn-<slug>`.

**Skip (most rules):** no `elysia` import, test/spec/stories, `allow`.

**Exception:** `bn-no-route-factory` is gated to `/modules/` or `/routes/` only (no import required).
...
## `elysia/bn-require-response-schema`
...
## `elysia/bn-require-route-export-name`
...
## `elysia/bn-require-route-schema`
...
## `elysia/bn-routes-index-mount-only`
