...
Plugin name: `elysia`. Ids: `elysia/vm-<slug>`.

**Skip (most rules):** no `elysia` import, test/spec/stories, `allow`.

**Exception:** `vm-no-route-factory` is gated to `/modules/` or `/routes/` only (no import required).
...
## `elysia/vm-require-response-schema`
...
## `elysia/vm-require-route-export-name`
...
## `elysia/vm-require-route-schema`
...
## `elysia/vm-routes-index-mount-only`
