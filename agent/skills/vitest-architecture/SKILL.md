---
description: "Enforce portable Vitest house style in apps/packages: per-workspace unit + integration projects, tests/ layout, scripts, passWithNoTests, fixtures/setup, and unit vs integration boundaries. Use when scaffolding tests, reviewing test layout, adding Vitest to a package, or when the user asks for vitest-architecture. Optional with-* overlays for testcontainers, Effect testing, Elysia handle, React happy-dom, typecheck projects, Storybook browser, live harnesses, and coverage. Not a Vitest API docs skill."
---
# Vitest Architecture
