...
5. For locale routing, native links/navigate, SSR `getLocale`/`getIntlayer`, SEO head, sitemap, and 404s, use [routing-ssr-seo.md](references/routing-ssr-seo.md).
6. Prefer `bun` / `bunx` in command examples. Prefer the official Start guide over Next.js or Solid Start docs — but **do not** copy Intlayer's `LocalizedLink` / `useLocalizedNavigate` examples; use native TanStack Router `Link` / `useNavigate` instead.
