...
- Prefer `useTable` plus explicit `tableFeatures()` for one-off tables. Use `createTableHook` when multiple tables share features, defaults, or standardized components. Use `stockFeatures` only as a kitchen-sink shortcut when tree-shaking does not matter yet.
...
- Prefer individually imported `filterFn_*`, `sortFn_*`, and `aggregationFn_*` entries in registry slots. Full `filterFns` / `sortFns` / `aggregationFns` objects still work but bundle every built-in.
